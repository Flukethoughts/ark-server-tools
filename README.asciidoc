...
Note the ports for rcon must be open for commands to be issued through arkmanager this however doesn't need to be open on the firewall if you don't want to allow rcon from outside the server.
...
[source,sh]
arkmanager install @instancename
